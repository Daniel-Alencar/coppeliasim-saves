...
Read the robot battery level from the float signal:

str(robotHandle) + "Battery"
Set the robot docking mode through the integer signal:

str(robotHandle) + "Docking"
Read the robot charging state through the integer signal:

str(robotHandle) + "Charging"
Read the charging-base IR beacon signal strength from the float signal:

str(robotHandle) + "signalStrength"
Read the charging-base relative angle from the float signal:

str(robotHandle) + "relativeAngle"
Receive robot velocity commands from ROS 2 and directly control the left and right wheel joints using the CoppeliaSim ZeroMQ Remote API.
